...
一个本地优先的视频字幕工具：转写视频语音，将字幕翻译成中文或英文，人工审核翻译文本，并把目标语言或双语字幕烧录进最终视频。
...
- 双语模式下会显示原文/目标字幕两条指示线，点击靠近哪条线就调整哪种语言
- 打开翻译校对窗口，编辑原文/目标字幕 segment，然后校对后渲染当前视频
...
- 使用 `--target-language en` 时为 `transcript/lecture.en.json`
- `transcript/lecture.bilingual.txt`

双语 TXT 会把每段原文和目标语言翻译放在一起，方便审核；最终样式字幕不依赖 TXT。
...
- `output/lecture/lecture.bilingual.ass`
- `output/lecture/lecture_zh_subs.mp4`
- 使用 `--target-language en` 时为 `output/lecture/lecture_en_subs.mp4`
